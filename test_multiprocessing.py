...
#===============================================================================
# This is a test script which I want to optimize. The bottleneck of the script
# performance is a function that calls the WOFOST model to be run. I have to
# optimize: the nested for-loops I use to loop over the input parameters and
#  maybe use multiprocessing to speed up the process. 
def main():
    global EUROSTATdir, folderpickle, detrend
#===============================================================================
# Define the main settings of the WOFOST runs
    ...
    # Period of time during which we have yield observations:
    start_year    = 1975
    end_year      = 2014
...
# Retrieve the observed yield and remove the technological trend 
    ...
    # we detrend the yield observations, because the long-term technological
    # trend cannot be caught by the crop model
...
#-------------------------------------------------------------------------------
# Optimize the yield gap factor parameter of the model
# This step is basically doing yield data assimilation

    optimum_yldgapf = optimize_yldgapf(crop_no, selected_grid_cells,
                                                selected_soil_types,
                                                opti_years)

#-------------------------------------------------------------------------------
# Do forward simulations using the optimum yield gap factor
...
#===============================================================================
def optimize_yldgapf(crop_no_, selected_grid_cells_, selected_soil_types_,
                   opti_years_):
#===============================================================================

    # 1- we add a timestamp to time the function
    print 'Started function at timestamp:', datetime.utcnow()
    ...
    print 'Finished function at timestamp:', datetime.utcnow()
